...
import com.aishiki.service.UserService;
...
public class UserController {
	...
	public String login(HttpSession session,String randStr,String account,String password) {
		String randStr2=(String) session.getAttribute("randStr");//从code.jsp取得session
			if(randStr2 != null && randStr2.equals(randStr)) {
				...
				if (!currentUser.isAuthenticated()) {
		            ...
		            } catch (UnknownAccountException ae) {//用户不存在
	...
	public String logout(HttpSession session) {
		...
		return "redirect:http://localhost:8989/graduation/login.html";
	}
